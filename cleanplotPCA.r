"cleanplot.pca" <- function(res.pca, ax1=1, ax2=2, point=FALSE, 
                            ahead=0.07, cex=0.7, labs, k, dfcut, cluster = FALSE) 
{
  ...
  if (point) {
    if (cluster) {
      ...
      points(sit.sc1[,ax1], sit.sc1[,ax2], col=1+c(1:k)[gr], pch=15+c(1:k)[gr])
      # text(res.pca, display="wa", choices=c(ax1, ax2), cex=cex, pos=3, scaling=1)
      text(res.pca, labels = labs, display="wa", choices=c(ax1, ax2), cex=cex, pos=3, scaling=1)
      } 
    else {
      points(sit.sc1[,ax1], sit.sc1[,ax2], pch=20)
      # text(res.pca, display="wa", choices=c(ax1, ax2), cex=cex, pos=3, scaling=1)
  ...
  if (point) {
    if (cluster) {
      points(sit.sc2[,ax1], sit.sc2[,ax2], col=1+c(1:k)[gr], pch=15+c(1:k)[gr])
      text(res.pca, labels = labs, display="wa", choices=c(ax1 ,ax2), cex=cex, pos=3)
    } else {
