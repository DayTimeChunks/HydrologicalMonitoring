...
p <- ggplot(data = soilGrApp, aes(x=Conc.mug.g.dry.soil, y=DD13C.comp))+
  ...
  annotate("text", x = 5.5, y = 3.7, label = as.character(r_label), parse = T, size = 5) +
  annotate("text", x = 5.5, y = 3.2, label = p_label, parse = T, size = 5) +
...
wsALL <- ggdraw() +
  ...
  draw_plot(wsALL_field_noLeg, x=0, y=.2, width = 0.939, height = .8) + # top
