...
p <- ggplot(data = subset(soilGrApp, !is.na(ngC.Label)), aes(x=Conc.mug.g.dry.soil, y=DD13C.comp))+
  ...
  geom_point(aes(group = ID, size = timeSinceApp)) + # , colour = Source)) +  # , shape = ngC.Label)) +
  theme_bw() +
  ...
  annotate("text", x = 5, y = 5, label = as.character(r_label), parse = T, size = 3.5) +
  annotate("text", x = 5, y = 4.5, label = p_label, parse = T, size = 3.5) +
...
type <- rep(c("CSIA", "DT50", "CSIA", "DT50", "MB", "MB"), 2)
...
balTidyType$type <- factor(balTidyType$type, levels = c("CSIA" , "MB", "DT50"))
levels(balTidyType$Sink)
levels(balTidyType$Month)


OutBars <- ggplot(data = balTidyType , aes(x=Month, y=measure, fill = Sink, ymin=SD1, ymax=SD2))+
  ...
  #xlab("Month") +
  facet_wrap(~type) +
  scale_fill_manual(values = c("#01665e", "#35978f", "#80cdc1", "#ec7014", "#fe9929", "#fec44f" ), # blue-orange
...
balAllplot <- ggdraw() +
  draw_plot(OutBars_noLeg, x=0.01, y=.5, width =  0.86, height = .5) + 
  draw_plot(SoilBars_noLeg, x=0.01, y = 0.0, width = 0.60, height = 0.5) +
  draw_plot(OutBars_Leg, x=0.74, y = 0.3, width = 0.1, height = 0.1) +
  draw_plot(SoilBars_Leg, x=0.70, y = 0.1, width = 0.1, height = 0.1) +
  draw_label("A", x= 0.11, y = .9, size = 12, fontface = "bold") +
  draw_label("D", x= 0.11, y = .39, size = 12, fontface = "bold") +
  draw_label("B", x= 0.37, y = .9, size = 12, fontface = "bold") +
  draw_label("E", x= 0.37, y = .39, size = 12, fontface = "bold") +
  draw_label("C", x= 0.63, y = .9, size = 12, fontface = "bold") 
...
waterIsoConc  <- ggplot(data = subset(waterClean, ngC.mean.diss > 5), aes(x=Conc.mug.L, y=DD13C.diss))+
  ...
  ylab(expression(paste({Delta~delta}^"13","C", ' (\u2030)'))) +
  xlab(expression(paste("Conc. S-Meto.  ", {({mu}*g / L)}))) +
  annotate("text", x = 22, y = 2.5, label = as.character(water_r_label), parse = T, size = 3.5) +
...
wsALL_lab <- ggplot(data = wstidier2, aes(x = Date, y = measure, group = Source) )+ 
  ...
  scale_y_continuous(
    ...
    sec.axis = sec_axis(trans = ~ (1-((1000 + d13Co + .)/(1000+d13Co))^(1000/epsilon_lab))*100 , 
                        name = element_blank(), breaks=c(20, 40, 60, 70, 80, 85, 90, 95) )# breaks=seq(20, 120, 15))
  )  + 
...
wsALL_field <- ggplot(data = wstidier2, aes(x = Date, y = measure, group = Source) )+ 
  ...
  scale_y_continuous(
    ...
    sec.axis = sec_axis(trans = ~ (1-((1000 + d13Co + .)/(1000+d13Co))^(1000/epsilon_field))*100 , 
                        name = "Degradation (%)", breaks=c(20, 40, 60, 70, 80, 85, 90, 95) )# breaks=seq(20, 120, 15))
  )  + 
  scale_color_manual(name= "Source", 
                      values = c("#F8766D", "#00BFC4", "#00BA38", "#B79F00", "#619CFF", "#F564E3",
                                 "#D55E00", "darkgreen", "dodgerblue") 
  ...
  scale_size_continuous(range = c(1, 3)) + 
  guides(col = guide_legend(nrow =  3), shape=guide_legend(nrow=2), size = guide_legend(nrow=3))# +
  # scale_shape_manual(name= )
...
wsALL <- ggdraw() +
  draw_plot(wsALL_field_noLeg, x=0, y=.2, width = 1, height = .8) + 
  draw_plot(wsALL_lab_noLeg, x=0, y = 0.2, width = 0.935, height = 0.8) +
  draw_label(label2, x= .89, y = .2, size = 8) +
  draw_label(label3, x= .94, y = .15, size = 8) +
  draw_plot(wsAll_field_Leg, x=0.2, y=0.0, width = 0.55, height = 0.15)
