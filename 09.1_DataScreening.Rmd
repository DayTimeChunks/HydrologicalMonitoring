...
mayBal$B.sd <- B.sd.may
mayBal$f.mean <- f.mean.may
mayBal$f.sd <- f.sd.may
...
juneBal$B.sd <- B.sd.june
juneBal$f.mean <- f.mean.june
juneBal$f.sd <- f.sd.june
...
type <- rep(c("CSIA", "Theoretical", "CSIA", "Theoretical", "MB", "MB"), 2)
...
ggplot(data = balTidyType , aes(x=factor(Month), y=measure))+
  geom_bar(stat = "identity", aes(fill = Sink), position = "dodge") +
  geom_errorbar(data = balTidyType, aes(x=factor(Month), y=measure, ymin=SD1, ymax=SD2),
  ...
  xlab("Month") +
  facet_wrap(~type) +
  scale_fill_manual(#values = c("#238b45", "#41ab5d", "#74c476", "#40004b", "#762a83", "#9970ab" ), # green-purple
                    ...
                    name= element_blank(), #"Mass Balance", # \n
                       ...
                       labels=c("Degr. (CSIA)", "Persist. (CSIA)", 
                                "Degr. (MB)", "S-MET Loads (MB)", 
                                "Degr. (DT50)" , "Persist. (DT50)"
                                 )) 
