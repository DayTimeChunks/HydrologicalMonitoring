...
p <- ggplot(data = soilGrApp, aes(x=Conc.mug.g.dry.soil, y=DD13C.comp))+
  ...
  geom_point(aes(group = Transect, colour = Transect, size = timeSinceApp)) +
  # theme_bw() +
  theme_minimal() +
...
```{r, echo = FALSE}
pPNG <- ggplot(data = soilGrApp, aes(x=Conc.mug.g.dry.soil, y=DD13C.comp))+
  geom_errorbar(aes(ymin = DD13C.comp - prop.d13C.SD, ymax = DD13C.comp + prop.d13C.SD)) +
  geom_errorbarh(aes(xmin = Conc.mug.g.dry.soil - Conc.ComSoil.SD, xmax = Conc.mug.g.dry.soil + Conc.ComSoil.SD)) +
  stat_smooth(data = subset(soilGrApp, Conc.mug.g.dry.soil < 8), 
              aes(x=Conc.mug.g.dry.soil, y=DD13C.comp), method = "lm", formula = y ~ poly(x, 2), se=F) +
  ...
  ylab(expression(paste({Delta~delta}^"13","C", ' (\u2030)'))) +
  xlab(expression(paste("S-met Soil Concentration  ", {({mu}*g / g~dry~wt.)}))) +
  annotate("text", x = 7.0, y = 4.7, label = as.character(r_label), 
           size = 5,
           parse = T) +
  annotate("text", x = 7.0, y = 4.2, label = p_label, 
           # size = 5,
           parse = T) +
  scale_size_continuous(range = c(1, 5), breaks= c(0, 10, 20, 30, 50), limits = c(0, 50)) + 
  scale_y_continuous(breaks=c(0, 1, 2, 3 , 4 ,5) ) + 
  # scale_size_continuous(range = c(1, 5)) +
  guides(size=guide_legend(nrow=1)) +
  # Rectangle Without labels 
  annotate("rect", xmin=0, xmax=8, ymin=-propagatedError, ymax=propagatedError, alpha=0.2) + 
  # Rectangle with labels 
  # annotate("rect", xmin=0, xmax=8, ymin=0, ymax=propagatedError, alpha=0.2) + 
  annotate("text", x = 4, y = -0.3, label= "italic(Dilution)", 
           size = 4.5,
           parse=T 
           ) +
  geom_segment(aes(x = 6, y = -0.5, xend = 2.5, yend = -0.5),
  ...
  annotate("text", 
           x = 4.0, y = 3.78, 
           label= "paste(\"(\", italic(Bio), \") \", italic(degradation) )", parse=T, 
           size = 4.5, 
           angle=332.5) +
  geom_segment(aes(x = 6, y = 2.2, xend = 2.5, yend = 4.5),
                   arrow = arrow(length = unit(1/2, 'picas'), type = "closed")) 
...
pPNG
# Without labels 
# ggsave(pPNG, filename = "images/DDvsConc_soils_B.png" , width = 8.7, height = 6 )#, units = "cm", scale = 1)
# With labels
# ggsave(pPNG, filename = "images/DDvsConc_soils_A.png" , width = 8.7, height = 6 )#, units = "cm", scale = 1)
```



### Field enrichment derivation (for error estimation)
...
# Save for SI Graphs (book 11.2)
write.csv2(WaterSoils, "Data/WaterSoils_R.csv", row.names = F)
...
write.csv2(wstidier2, "Data/OutletSoilsPlot_R.csv", row.names = F)
...
keepConctest <- c( "Date.ti",
                   "Conc.mug.L" , 
                   "Conc.mug.g.dry.soil.S" , "Conc.mug.g.dry.soil.T", "Conc.mug.g.dry.soil.N") 

wsStatTest <- WaterSoils[, colnames(WaterSoils) %in% keepDDtest]
wsStatTestConc <- WaterSoils[, colnames(WaterSoils) %in% keepConctest]

mwsStatTest <- melt(wsStatTest, id="Date.ti")
mwsStatTestConc <- melt(wsStatTestConc, id="Date.ti")
...
## Box plot outlet vs soils ()

```{r}
mwsStatTest$Group4 <- ifelse(mwsStatTest$variable == "diss.d13C.x" & # Outlet
                                   mwsStatTest$Date.ti > as.POSIXct('2016-06-05 00:06:00', tz = 'EST'), "Outlet(Late)",
                                 ifelse(mwsStatTest$variable == "diss.d13C.x" & 
                                   mwsStatTest$Date.ti <= as.POSIXct('2016-06-05 00:06:00', tz = 'EST'), "Outlet(Early)",
                                 # South
                                 ifelse(mwsStatTest$variable == "comp.d13C.South" & 
                                   mwsStatTest$Date.ti < as.POSIXct('2016-06-05 00:06:00', tz = 'EST'), "South(Early)",
                                   ifelse(mwsStatTest$variable == "comp.d13C.South" & 
                                     mwsStatTest$Date.ti > as.POSIXct('2016-06-05 00:06:00', tz = 'EST'), "South(Late)",
                                # Valley
                                     ifelse(mwsStatTest$variable == "comp.d13C.Talweg" & 
                                   mwsStatTest$Date.ti < as.POSIXct('2016-06-05 00:06:00', tz = 'EST'), "Valley(Early)",
                                   ifelse(mwsStatTest$variable == "comp.d13C.Talweg" & 
                                     mwsStatTest$Date.ti > as.POSIXct('2016-06-05 00:06:00', tz = 'EST'), "Valley(Late)",
                                 # North
                                     ifelse(mwsStatTest$variable == "comp.d13C.North" & 
                                   mwsStatTest$Date.ti < as.POSIXct('2016-06-05 00:06:00', tz = 'EST'), "North(Early)",
                                   ifelse(mwsStatTest$variable == "comp.d13C.North" & 
                                     mwsStatTest$Date.ti > as.POSIXct('2016-06-05 00:06:00', tz = 'EST'), "North(Late)", NA
                                  ))))))))



boxplot(mwsStatTest$value ~ mwsStatTest$Group4)

mwsStatTestConc$Group4 <- ifelse(mwsStatTestConc$variable == "Conc.mug.L" & # Outlet
                                   mwsStatTestConc$Date.ti > as.POSIXct('2016-06-05 00:06:00', tz = 'EST'), "Outlet(Late)",
                                 ifelse(mwsStatTestConc$variable == "Conc.mug.L" & 
                                   mwsStatTestConc$Date.ti <= as.POSIXct('2016-06-05 00:06:00', tz = 'EST'), "Outlet(Early)",
                                 # South
                                 ifelse(mwsStatTestConc$variable == "Conc.mug.g.dry.soil.S" & 
                                   mwsStatTestConc$Date.ti < as.POSIXct('2016-06-05 00:06:00', tz = 'EST'), "South(Early)",
                                   ifelse(mwsStatTestConc$variable == "Conc.mug.g.dry.soil.S" & 
                                     mwsStatTestConc$Date.ti > as.POSIXct('2016-06-05 00:06:00', tz = 'EST'), "South(Late)",
                                # Valley
                                     ifelse(mwsStatTestConc$variable == "Conc.mug.g.dry.soil.T" & 
                                   mwsStatTestConc$Date.ti < as.POSIXct('2016-06-05 00:06:00', tz = 'EST'), "Valley(Early)",
                                   ifelse(mwsStatTestConc$variable == "Conc.mug.g.dry.soil.T" & 
                                     mwsStatTestConc$Date.ti > as.POSIXct('2016-06-05 00:06:00', tz = 'EST'), "Valley(Late)",
                                 # North
                                     ifelse(mwsStatTestConc$variable == "Conc.mug.g.dry.soil.N" & 
                                   mwsStatTestConc$Date.ti < as.POSIXct('2016-06-05 00:06:00', tz = 'EST'), "North(Early)",
                                   ifelse(mwsStatTestConc$variable == "Conc.mug.g.dry.soil.N" & 
                                     mwsStatTestConc$Date.ti > as.POSIXct('2016-06-05 00:06:00', tz = 'EST'), "North(Late)", NA
                                  ))))))))
boxplot(mwsStatTestConc$value ~ mwsStatTestConc$Group4)
