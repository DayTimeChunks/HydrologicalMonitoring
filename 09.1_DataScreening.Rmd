...
wstidier <- wsSmall %>%
  gather(measure, value, -Date, -Event) %>% # Melts data frame
...
wsALL_lab <- ggplot(data = wstidier2, aes(x = Date, y = measure, group = Source) )+ 
  ...
  geom_point(data=subset(wstidier2, Source == 'Outlet'), aes(shape = Type, colour = Source)) +
...
wsAll__field_Leg  <- get_legend(wsALL_field)
...
wsALL <- ggdraw() +
  ...
  draw_plot(wsAll__field_Leg, x=0.2, y=0.0, width = 0.6, height = 0.2)
