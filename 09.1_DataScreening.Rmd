...
# names(WaterSoils)
wsSmall <- WaterSoils[c("Date.ti", "WeekSubWeek", "ID.N", "Event.x",  
...
wstidier <- wsSmall %>%
  gather(measure, value, -Date, -IDSoil, -Event, -Week, 
         -Qmax, -Qmean, 
         -CumRain, -RainInt,
...
wsALL_lab <- ggplot(data = wstidier2, aes(x = Date, y = measure, group = Source) )+ 
  ...
  # geom_point(data=subset(wstidier2, Source == 'Outlet'), aes(shape = Type, colour = Source, size = CumRain)) + 
  # geom_point(data=subset(wstidier2, Source == 'Outlet'), aes(shape = Type, colour = Source, size = noEventHrsIni)) +
