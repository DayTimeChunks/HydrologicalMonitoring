...
soilGroups <- subset(soilGroups, comp.d13C.SD <= 0.75)
...
soilGrApp <- subset(soilGrApp, comp.d13C.SD <= 0.75)
...
soilGrApp$ftot <- ifelse(soilGrApp$ftot > 1, 1, soilGrApp$ftot)

soilGrApp$yRaleigh <- log((1000+d13Co+soilGrApp$DD13C.comp)/(1000+d13Co))
soilGrApp$xRaleigh <- log(soilGrApp$ftot)/1000


model<-lm(yRaleigh~xRaleigh, data= soilGrApp, subset=(N_compsoil >= 3)) 
cofsoil <- as.numeric(coef(model)[2])
se <- summary(model)$coef[[4]]

minXsoil <- confint(model, "xRaleigh", level = 0.95)[1]
maxXsoil <- confint(model, "xRaleigh", level = 0.95)[2]

CI95soil = maxXsoil - cofsoil

lab <- sprintf(" epsilon == %0.2f ", cofsoil)
labSE <- sprintf("\u00B1 %0.2f ", CI95soil)
labSE2 <- sprintf("± %0.2f ", CI95soil)
...
rayleigh <- 
  ...
  #         label = as.character(expression(paste( "\u00B1" , 0.49))), parse = T, size = 3.0) # +
...
soilGrApp$Fdil <- ifelse(soilGrApp$Fdil < 1, 1, soilGrApp$Fdil)
...
Re-calculating the enrichment factor based on dilution processes (i.e. new f):

```{r}

soilGrApp$xRaleighDeg <- log(soilGrApp$fdeg)

modelDegField<-lm(yRaleigh~xRaleighDeg, data= soilGrApp, subset=(N_compsoil >= 3)) 
summary(modelDegField)

cofDegField <- as.numeric(coef(modelDegField)[2])

minXDegField <- confint(modelDegField, "xRaleighDeg", level = 0.95)[1]
maxXDegField <- confint(modelDegField, "xRaleighDeg", level = 0.95)[2]

CI95degField = maxXDegField - cofDegField


rayleighCorrect <- 
  ggplot(data = subset(soilGrApp, ( Wnum > 0 & N_compsoil >= 2 & !is.na(yRaleigh) )), 
         aes(x=xRaleighDeg, y=yRaleigh)) +
  geom_point(aes(group = ID, size = timeSinceApp.NoSo ))+ #, colour = Source)) +  #, shape = ngC.Label)) +
  theme_bw() +
  scale_size_continuous(range = c(1, 5)) +
  labs(size="Days post appl.", colour="Source") + #, shape = "Mass Carbon") +
...
watersMassBal$f.Field <- 100 - (watersMassBal$B.diss.Field) # + watersMassBal$SMout_prc)
watersMassBal$f.min.Field <- 100 - (watersMassBal$B.diss.max.Field)# + watersMassBal$SMout_prc)
watersMassBal$f.max.Field <- 100 - (watersMassBal$B.diss.min.Field) # + watersMassBal$SMout_prc)
...
waterIsoConc  <- ggplot(data = subset(waterClean, ngC.mean.diss > 5), aes(x=Conc.mug.L, y=DD13C.diss))+
  ...
  annotate("text", x = 7.5, y = 2.7, label = as.character(water_r_label), parse = T, size = 3.5) +
  annotate("text", x = 7.5, y = 2.3, label = water_p_label, parse = T, size = 3.5)
...
WaterSoils$BulkCatch.d13
WaterSoils$BulkCatch.DD13
...
keepWS <- c("Date.ti", "WeekSubWeek", "ID.N", "Event.x",
            ...
            "BulkCatch.DD13", "BulkCatch.d13.SD" 
...
wsTest$BulkCatch.DD13 <- na.locf(wsTest$BulkCatch.DD13)
...
cor.test(wsTest$BulkCatch.DD13, wsTest$DD13C.diss, method = "pearson", use = "pairwise.complete.obs")
...
wstidier2 = subset(wstidier, SD <= 0.75) #& Source != "Bulk"  ) #& Date < as.POSIXct('2016-06-14 08:04:00'))
...
wsALL_field <- ggplot(data = wstidier2, aes(x = Date, y = measure, group = Source) )+ 
  ...
  # Water
  stat_smooth(data=subset(NoBASE, #wstidier2, #NoBASE,
                          (Source == "Outlet" & Event > 1 & Type == "Dissolved")), 
              method = "lm", formula = y ~ poly(x, 2), se = T, aes(colour = 'Outlet'), alpha = 0.2, size=0.2) +
  # North
  stat_smooth(data=subset(wstidier2,
...
dissolved <- subset(wstidier2, Location == "diss" &
...
# May 12 event
eventMay12 <- subset(dissolved, Date >= as.POSIXct("2016-05-12 06:34:00", tz = "EST") & 
                       Date <= as.POSIXct("2016-05-13 12:06:00", tz = "EST"))

# Inspect the required mm / day
rainMay12 <- subset(rainDay, Month == "May" & (Day == 12 | Day == 13))
eventMay12$Intensity <- c(20.0, 20.0, 8.4)
eventMay12$Time <- c(0, 3, 24)
eventMay12$Event <- rep("May 12", 3)
eventMay12$Approach <- rep("Outlet, 2016", 3)

# May 30 event
eventMay30 <- subset(dissolved, Date >= as.POSIXct("2016-05-30 05:48:00", tz = "EST") & 
                       Date <= as.POSIXct("2016-05-31 12:00:00", tz = "EST"))

rainMay30 <- subset(rainDay, Month == "May" & (Day == 30 | Day == 31))
eventMay30$Intensity <- c(20.4, 20.4, 0)
eventMay30$Time <- c(0, 6, 24)
eventMay30$Event <- rep("May 30", 3)
eventMay30$Approach <- rep("Outlet, 2016", 3)

# June 2 event
eventJune2 <- subset(dissolved, Date >= as.POSIXct("2016-06-02 12:58:00", tz = "EST") & 
                       Date < as.POSIXct("2016-06-03 12:06:00", tz = "EST")) 

rainJune2 <- subset(rainDay, Month == "June" & Day == 2)
eventJune2$Intensity <- c(5)
eventJune2$Time <- c(0)
eventJune2$Event <- rep("June 2", 1)
eventJune2$Approach <- rep("Outlet, 2016", 1)

eventJune3 <- subset(dissolved, Date >= as.POSIXct("2016-06-03 12:06:00", tz = "EST") & 
                       Date <= as.POSIXct("2016-06-07 12:00:00", tz = "EST"))

rainJune3 <- subset(rainDay, Month == "June" & (Day > 2 & Day <= 7 ))
eventJune3$Intensity <- c(15.2, 18.0, 18.0, 0.8)
eventJune3$Time <- c(0, 20, 23, 96)
eventJune3$Event <- rep("June 3", 4)
eventJune3$Approach <- rep("Outlet, 2016", 4)

eventsField <- rbind(eventMay12, eventMay30, eventJune3) # eventJune2, 

names(eventsField)
eventsField <- eventsField[c("Time", "Intensity", "measure", "SD" ,  "Event", "Approach")]


Time <- c(0, 4, 6, 12, 24, 48, 72, 96)
Intensity <- c(30, 0, 0, 0, 0, 0, 0, 0)
measure <- c(0, 2, 3, 3.2, 3.4, 3.5, 3.5, 3.5)
SD <- rep(NA, 8)
Event <- rep("Simulation", 8)
Approach <- rep("Simulation (Lutz et al., 2013)", 8)

events <- data.frame(Time, Intensity, measure, SD, Event, Approach)

allEvents <- rbind (events, eventsField)

LutzEvents <- ggplot(data = allEvents, aes(x= Time, y=measure, colour = Event))+
  theme_bw() +
  ...
  guides(size = guide_legend(order = 4, 
                             title=expression("Intensity " ~ (mm/d) ), 
