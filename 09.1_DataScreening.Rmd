...
model<-lm(yRaleigh~xRaleigh, data= soilGrApp, subset=(!is.na(ngC.Label))) # & ngC.mean >= 5)) 
...
model2<-lm(yRaleigh~xRaleigh, data= soilGrApp, subset=(!is.na(ngC.Label) & DD13C.comp < 4)) # & ngC.mean >= 5)) 
summary(model2)
...
rayleigh <- 
  ggplot(data = subset(soilGrApp, ( Wnum > 0 & !is.na(ngC.Label) & !is.na(yRaleigh) )), 
...
```{r}
DBmodel<-lm(log(fdeg)~log(fdil), data= soilGrApp, subset=(!is.na(fdil))) # & ngC.mean >= 5)) 
...
waters <- waters[ , (names(waters) %in% keepWaterMB)]

# Get last 5 rows, omit NA's, will return rows only where B.diss was not NA
watersMassBal <- na.omit(waters[ , (names(waters) %in% keepWaterMB)])
...
str(WaterSoils)
names(WaterSoils)
keepWS <- c("Date.ti", "WeekSubWeek", "maxQ", "AveDischarge.m3.h",
...
names(wsSmall)
wsSmall <- wsSmall[c("Date.ti", "Event.x", "WeekSubWeek", "maxQ", "AveDischarge.m3.h",
            "DD13C.diss", "SD.d13C.x",
            "DD13C.filt", "filt.SD.d13C" ,
            "DD13C.Talweg", "comp.d13C.SD.Talweg", 
            "DD13C.South", "comp.d13C.SD.South", 
...
names(wsSmall) <- c("Date", "Event", "Week", "Qmax", "Qmean",
...
wstidier <- wsSmall %>%
  gather(measure, value, -Date, -Event, -Week, -Qmax, -Qmean) %>% # Melts data frame
