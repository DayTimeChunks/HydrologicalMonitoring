...
To compute initial concentration needed for Rayleigh calculations, the application rates are used to derive the respective concentration at each plot $C_i$, plot area $A$ and the effective transect area $A_tr$ (i.e. proportional to sampling points along transect, not total area represented by transect within the catchment). 

Note that initial concentrations at each transect are then extrapolated to the catchment to calculate initial catchment concentrations (bulk), which in turn do take into account the full catchment area.
