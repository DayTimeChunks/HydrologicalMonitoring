...
if (SAVE){
  if (PC){
    ggsave(exp, 
       filename = "D:/Documents/these_pablo/WriteUp/Alteck_PNAS_LaTeX/images/DDvsDays.pdf", 
...
```{r}
g = ggplot(data = enrich, aes(x=C.SM, y=DD13m))+
  geom_errorbar(aes(ymin = DD13m - SD, ymax = DD13m + SD)) +
  geom_point(aes(group = Type, colour = Type, size = Days)) + 
  theme_minimal() +
  # stat_smooth(method = "lm", formula = y ~ x, se=F) +
  ylab(expression(paste({Delta~delta}^"13","C", ' (\u2030)'))) 
