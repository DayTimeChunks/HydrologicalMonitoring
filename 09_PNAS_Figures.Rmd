...
weeklySoil$ngC.Label <- ifelse(weeklySoil$ngC.mean <10, "< 10 ng", 
                               ifelse( weeklySoil$ngC.mean >= 10 & weeklySoil$ngC.mean < 15, "< 15 ng", "> 15 ng"))
wk <- weeklySoil
# wk <- na.omit(weeklySoil)
wk$Application <- ifelse(wk$Date.ti == as.POSIXct('2016-05-17 09:16:00', tz = "EST") & wk$Transect == "T", 0, 
                         ifelse(wk$Date.ti == as.POSIXct('2016-03-25 00:04:00', tz = "EST") & wk$Transect == "T", 0, 
                                ifelse(wk$Date.ti == as.POSIXct('2016-04-05 15:08:00', tz = "EST") & wk$Transect == "T", 0,
                                       ifelse(wk$Date.ti == as.POSIXct('2016-04-14 13:52:00', tz = "EST") & wk$Transect == "T", 0, NA))))
...
deltaTime = ggplot(wk, aes(Date.ti, DD13C.comp)) +
  ...
  geom_point(aes(Date.ti, Application, shape = "Application"), data = wk,  legend.title = element_blank()) +
  # geom_point(weeklySoil[1:48, ], aes(x=Date.ti, y=DD13C.comp, colour=Transect, group = Transect)) +
