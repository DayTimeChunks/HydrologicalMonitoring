...
bulkDegDF <-  soilsOut[, c("ti", "WeekSubWeek", "ID.S" ,"BulkCatch.d13", "B.mean.bulk", "B.bulk", "B.min.bulk")]
bulkDegDF <- na.omit(bulkDegDF)
bulkDegDF$Transect <- "Bulk" 
...
bulkDegDF <- bulkDegDF[, c("Date.ti", "Transect", "B.mean.com", "Week")]

wSoil <- weeklySoil[, c("Date.ti", "Transect", "B.mean.com")]
wSoil$Week <- NA
...
levels(wSoilBulkDeg$Transect)[levels(wSoilBulkDeg$Transect)=="N"] <- "North"
levels(wSoilBulkDeg$Transect)[levels(wSoilBulkDeg$Transect)=="T"] <- "Talweg"
levels(wSoilBulkDeg$Transect)[levels(wSoilBulkDeg$Transect)=="S"] <- "South"
levels(wSoilBulkDeg$Transect)[levels(wSoilBulkDeg$Transect)=="Bulk"] <- "Bulk Fraction"

levels(wSoilBulkDeg$Transect)
wSoilBulkDeg$Transect <- factor(wSoilBulkDeg$Transect, levels = c("North", "Talweg", "South", "Bulk Fraction"))


# View(weeklySoil)
Bsoil2 = ggplot(wSoilBulkDeg, aes(x=Date.ti, y=B.mean.com, colour=Transect, group = Transect)) +
  ...
  geom_point() +
  scale_colour_manual(values=c("#F8766D", "#7CAE00", "#619CFF", "black")) +
  stat_smooth(data=subset(wSoilBulkDeg, Transect == "Bulk Fraction"), method = "lm", formula = y ~ poly(x, 2), se=T) +
  # geom_point(aes(x=Date.ti, y=B.comp, colour=Transect, group = Transect)) +
  # Theme and axes
  theme_bw() +
  ylab("Degr. %") +
  ...
  # stat_smooth(data=subset(weeklySoil[4:39, ]), method = "lm", formula = y ~ poly(x, 2), se = F) 
  # stat_smooth(method = "lm", formula = y ~ x, se=FALSE)
  geom_text_repel(aes(y=B.mean.com, label=Week),
                  size = 3,
                  arrow = arrow(length = unit(0.005, 'npc'), type = "closed"),
                  force = 0.9, 
...
legend_deg = get_legend(Bsoil2 + guides(colour = guide_legend(title.position = "top", nrow = 4, byrow = T)))
...
fig2 <- ggdraw() +
  ...
  draw_plot(legend_deg, x=0.5, y =0.7, width = 0.65, height = 0.25) +
  draw_plot(legend_mass, x=0.5, y=0.2, width = 0.75, height = 0.3)

fig2

ggsave(fig2, filename = "BvsMassBal.tiff", height = 12, width = 17.8, units = 'cm')
