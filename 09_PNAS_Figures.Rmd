...
deltaTime = ggplot(na.omit(weeklySoil), aes(Date.ti, DD13C.comp)) +
  ...
  geom_point(aes(shape = Transect, colour = ngC.Label)) +
...
deltaTime = ggplot(wk, aes(Date.ti, DD13C.comp)) +
  ...
  # Smooth linear models
  #stat_smooth(data = weeklySoil, aes(group = Transect, colour = Transect), method = "lm", se=FALSE) +
  stat_smooth(data = weeklySoil, method = "lm", se=T) +
  # stat_smooth(data = subset(weeklySoil[1:34, ]), method = "lm", formula = y ~ poly(x, 2), se=FALSE) +
  # stat_smooth(method = "lm", formula = y ~ poly(x, 2)) +
  # stat_smooth(data=subset(weeklySoil[1:34, ]), method = "lm", formula = y~x, se=F) +
  # stat_smooth(data=subset(weeklySoil[1:34, ]), aes(group = Transect, colour = Transect), method = "lm", formula = y~x, se=F) +
  # stat_smooth(data=subset(weeklySoil[18:36, ]), method = "lm", formula = y~x, se=F) +
  ...
  geom_text_repel(data = subset(weeklySoil, Wnum < 10 & Wnum > 6), aes(label=Wnum, colour = Transect), 
...
names(AOdf)[names(AOdf) == "Event"] <- "Peak"
AOdf$Events <- as.factor(c("0-1", "0-2", "0-3",
                         "1-1", "1-2", "1-3", 
                         "2-1", "2-2", "2-3", 
                         "3-1", 
                         "4-1", "4-2", "4-3", "4-4", "4-5",
...
Split2 <- strsplit(as.character(AOdf$Events), "-", fixed = T)
AOdf$Event <- as.factor(sapply(Split2, "[", 1))
...
data <- AOdf[which(AOdf$Sampled == 'Sampled'), ]
data$Row <- seq.int(nrow(data))

conc1 <- ggplot(data, aes(x=ti, y=Conc.mug.L)) +
  ...
  #stat_smooth(data=subset(data[19:30, ]), method = "lm", formula = y ~ x) +
  # Text
  # Application W9
  # annotate("text", 
  ...
  #             arrow = arrow(length = unit(0.2, "cm"))) +
  geom_text_repel(aes(label=Events, color = factor(Event)), # WeekSubWeek or Weeks
                  size = 3,
                  arrow = arrow(length = unit(0.005, 'npc'), type = "closed"),
                  force = 0.5, 
                  point.padding = unit(0.5, 'lines'), 
...
data1 <- data[ , c("ti", "SimOutESA.g", "SimOutOXA.g", "SimOutSmeto.g", "SimMELsm.g")]

limits_conc <- aes(ymin=Conc.mug.L-Conc.SD, ymax=Conc.mug.L+Conc.SD, color = Event, group = Event)

mass1 <- ggplot(data) +
  geom_point(aes(x=ti, y=SimOutSmeto.g)) +
  geom_point(aes(x=ti, y=SimMELsm.g)) + 
...
limits_DdCwater <- aes(ymin=diss.d13C-se.d13C-initialDelta, ymax=diss.d13C+se.d13C-initialDelta, color = Event, group = Event)

iso2 <- ggplot(data, aes(x=ti, y=DD13C.diss)) +
  # Error bars
  ...
  geom_text_repel(aes(label=Events, color = factor(Event)),
                  size = 3,
                  arrow = arrow(length = unit(0.005, 'npc'), type = "closed"),
                  force = 0.5, 
                  point.padding = unit(0.5, 'lines'), 
