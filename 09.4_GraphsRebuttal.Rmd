...
# Lab
coflab_label <- sprintf("epsilon == %0.1f", cof_bio)
...
# Field
coffield_label <- sprintf("epsilon == %0.1f", cof_field)
if (p_field < 0.0001){
...
if (SAVE){
  if (PC){
    ...
    ggsave(mixALL, 
     # filename = "/Users/DayTightChunks/Documents/PhD/Writeups/PNAS/Alteck_PNAS_LaTeX/images/LabFieldDDConc.pdf", 
      device=cairo_pdf, dpi = 600,  scale = 1, # )# ,
       width = 8.7, height = 3)
  }
}
...
wsall <- ggplot(data = wstidier2, aes(x = Date, y = measure, group = Source, shape = Source, colour = Source) )+ 
  ...
  annotate("text", x = as.POSIXct('2016-03-28 08:04:00'), y = 0, 
           label = as.character(expression(paste( "+"))), parse = T, size = 7.0) +
  ...
  annotate("text", x = as.POSIXct('2016-05-25 08:04:00'), y = 0, 
           label = as.character(expression(paste( "+"))), parse = T, size = 7) +
  # Title applics
  annotate("text", x = as.POSIXct('2016-04-05 08:04:00'), y = 4, 
           label = as.character(expression(paste( "+"))), parse = T, size = 7.0) +
   annotate("text", x = as.POSIXct('2016-04-12 08:04:00'), y = 4, 
...
if (MODEL){
 ...
 stat_smooth(data=subset(wstidier2,
              ...
              method = "lm", formula = y ~ x, se = F, aes(colour = 'Outlet'), alpha = 0.9, size=0.2) +
  # Bulk
  stat_smooth(data=subset(wstidier2,
                          (Source == "Bulk" )), #| Source == "South" )), 
              method = "lm", formula = y ~ x, se = F, aes(colour = 'Bulk'), alpha = 0.2, size=0.2)
}
