...
epsilon_lab = -2.2 # ± 0.4
dE <- 0.4
epsilon_max = epsilon_lab + dE 
epsilon_min = epsilon_lab - dE
...
type <- rep(c("CSIA - Soils", "CSIA - Outlet", 
              ...
              "Mass Balance - Soils", 
              "Predicted (half-life)", 
              "Mass Balance - Outlet", "Mass Balance - Outlet", "Mass Balance - Soils"), 2)
...
OutBars <- ggplot(data = subset(ajTidy, 
  ...
  scale_fill_manual(#values = c("#01665e",  "#ec7014",   "#35978f",  "#fe9929", "#80cdc1", "#fec44f"), # blue-orange
                    ...
                    name= "Outlet" ,# element_blank(), #"Mass Balance", # \n
                       breaks=c("Bw", "fw" , 
                                "TPout" , "SMout" #, 
                                #"Deg", "Rem" 
                                ),
...
theoBars <- ggplot(data = subset(ajTidy, type == 'Predicted (half-life)') , 
                  aes(x=Month, y=measure, fill = Sink, ymin=SD1, ymax=SD2))+
  geom_bar(stat = "identity", position = "dodge", width = 0.5) +
  geom_errorbar(#aes(ymin=SD1, ymax=SD2),
                  width=.3 , # ) + #,                    # Width of the error bars
...
if (SAVE){
  if (PC){
      ggsave(balAllplot, 
           filename = "D:/Documents/these_pablo/WriteUp/Alteck_PNAS_LaTeX/images/balAllplot.pdf", 
