...
waterSmall$Sequence <- ifelse(is.na(waterSmall$Peak), 2, 1)
...
res.man <- manova(cbind(DD13C.diss.x, TotSMout.g.x, MELsm.g.x, 
                         Conc.mug.L, OXA_mean, ESA_mean) ~ Cluster, data = bindedGroups.K2.XY)
### Look to see which differ
summary.aov(res.man)

plist <- c(summary.aov(res.man)[[1]][["Pr(>F)"]][1], 
           summary.aov(res.man)[[2]][["Pr(>F)"]][1],
           summary.aov(res.man)[[4]][["Pr(>F)"]][1])

# Melt & but remove "Events" variable first
meltGroupK2 <- melt((bindedGroups.K2.X[ , !(names(bindedGroups.K2.X) %in% "Events")]), id=c("Cluster"))
