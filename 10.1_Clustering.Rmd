...
watSoilMerged$DD.Bulk.imp <- watSoilMerged$DD13.Bulk
if (is.na(watSoilMerged$DD.Bulk.imp[4])){
  watSoilMerged$DD.Bulk.imp[1:4] <- 1
} 
if (is.na(watSoilMerged$DD.Bulk.imp[5])){
  watSoilMerged$DD.Bulk.imp[5] <- 1
} 
watSoilMerged$DD.Bulk.imp[1:nrow(watSoilMerged)-1] <- na.approx(watSoilMerged$DD.Bulk.imp)

watSoilMerged$DD.diss.norm <-  watSoilMerged$DD13C.diss/watSoilMerged$DD.Bulk.imp # Losses values where no BULK was obtained
...
includeWater <- c(
  ...
  "DD13.Bulk", # "DD.Bulk.imp",
  "DD.diss.norm",  "DD.diff.prc",
  "TotSMout.g", "MELsm.g",
  "TPs.g", "DissOXA.g", "DissESA.g",
  # "Cl.mM", "NO3..mM",
  "ExpMES.Kg", 
  # Independent/event variables
  "DIa", "DIb", "maxQ", "chExtreme" ,
...
if ( class(waterX[, 1])== "factor") {
  # Hellinger
  water.hell <- decostand(waterX[, 2:ncol(waterX)], "hellinger", na.rm=T, MARGIN = 2)
  # waterXY.hell.nona <- waterXY.hell[complete.cases(waterXY.hell),]
  
  # Normalize to 1
  # make margin sum of squares equal to one (default MARGIN = 1)
  water.norm <- decostand(waterX[, 2:ncol(waterX)], "norm", na.rm=T, MARGIN = 2) # Columns
...
clusterGroups.norm = cutree(water.norm.clust, k = 2)

# No. of rows/observations
length(clusterGroups.hell) == lengths(waterXY)[1]
#length(clusterGroups.norm) == lengths(waterXY)[1]

length(clusterGroups.hell[clusterGroups.hell == 1])
length(clusterGroups.hell[clusterGroups.hell == 2])
  
bindedGroups.K2.X <- cbind2(clusterGroups.hell, waterXY)
bindedGroups.K2.X.norm <- cbind2(clusterGroups.norm, waterXY)

names(bindedGroups.K2.X)[names(bindedGroups.K2.X) == "x"] <- "Cluster"
names(bindedGroups.K2.X.norm)[names(bindedGroups.K2.X.norm) == "x"] <- "Cluster"

bindedGroups.K2.X$Cluster <- as.factor(bindedGroups.K2.X$Cluster)
bindedGroups.K2.X.norm$Cluster <- as.factor(bindedGroups.K2.X.norm$Cluster)

#bindedGroups.K3.X <- cbind2(clusterGroups.norm, waterXY)
bindedGroups.K2.XY <- merge(bindedGroups.K2.X, responseWater, by = "Events")
bindedGroups.K2.XY.norm <- merge(bindedGroups.K2.X.norm, responseWater, by = "Events")

#class(bindedGroups.K2.XY$x)
#group1 <- subset(bindedGroups.K2.XY, x == 1)
#group2 <- subset(bindedGroups.K2.XY, x == 2)

#x <- group1$DD13C.diss.x
#y <- group2$DD13C.diss.x
...
colnames(bindedGroups.K2.XY.norm)

res.man <- manova(cbind(DD13C.diss.x, DD.diss.norm, # Percent is exactly the same: DD.diff.prc,
                        TotSMout.g.x, MELsm.g.x,  Conc.mug.L, OXA_mean, ESA_mean, 
                        ExpMES.Kg,
                        # responses
                        maxQ, Duration.Hrs, AveDischarge.m3.h, Volume.m3, DIa
                        ) 
                  ~ Cluster, data = bindedGroups.K2.XY)
                  #~ Cluster, data = bindedGroups.K2.XY.norm)

# Test for normality
shapiro.test(bindedGroups.K2.XY$Conc.mug.L)
shapiro.test(bindedGroups.K2.XY$TotSMout.g.x)
shapiro.test(bindedGroups.K2.XY$OXA_mean)
shapiro.test(bindedGroups.K2.XY$DD.diss.norm)
shapiro.test(bindedGroups.K2.XY$DD13C.diss.x) # Not significant

n1 <- lengths(subset(bindedGroups.K2.XY, !is.na(DD.diss.norm) & Cluster == 1))[[1]]
n2 <- lengths(subset(bindedGroups.K2.XY, !is.na(DD.diss.norm) & Cluster == 2))[[1]]
#n3 <- lengths(subset(bindedGroups.K2.XY, !is.na(DD.diss.norm) & Cluster == 3))[[1]]
#n4 <- lengths(subset(bindedGroups.K2.XY, !is.na(DD.diss.norm) & Cluster == 4))[[1]]


# boxplot(bindedGroups.K2.XY$DD.diss.norm)
...
meltGroupK2 <- melt((bindedGroups.K2.XY[ , !(names(bindedGroups.K2.XY) %in% dropBox)]), id=c("Cluster"))
...
km2     <- kmeans(scores.hell, centers=4, nstart=10)
km3     <- kmeans(scores.norm, centers=2, nstart=10)
