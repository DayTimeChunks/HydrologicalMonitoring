...
soils.N <- soils.N[, c("Date.ti", "B.comp", "MassSoil.g", "comp.d13C", "comp.d13C.SD", "comp.d13C.SE", "ID")]
colnames(soils.N) <- c("ti",  "B.comp.North", "MassSoil.g.North", "comp.d13C.North", "comp.d13C.SD.North", "comp.d13C.SE.North",  "ID.N")

# Talweg
soils.T <- subset(soils, soils$Transect == "T")
soils.T <- soils.T[, c("Date.ti", "B.comp", "MassSoil.g", "comp.d13C", "comp.d13C.SD", "comp.d13C.SE", "ID")]
colnames(soils.T) <- c("ti",  "B.comp.Talweg", "MassSoil.g.Talweg", "comp.d13C.Talweg", "comp.d13C.SD.Talweg", "comp.d13C.SE.Talweg", "ID.T")

# South
soils.S <- subset(soils, soils$Transect == "S")
soils.S <- soils.S[, c("Date.ti", "B.comp", "MassSoil.g", "comp.d13C", "comp.d13C.SD", "comp.d13C.SE", "ID")]
colnames(soils.S) <- c("ti",  "B.comp.South", "MassSoil.g.South", "comp.d13C.South", "comp.d13C.SD.South", "comp.d13C.SE.South","ID.S")

```

# Total Catchment Mass and Bulk Isotopes
...
levels(pg$variable)[levels(pg$variable)=="CatchMassSoil.g"] <- "Catchment Mass (1cm Soil)"

# Change the order:
levels(pg$variable)
pg$variable <- factor(pg$variable, levels = c("Applied Cum. (Survey)", "Catchment Mass (1cm Soil)", "Dissolved Cum. (Outlet)", "Sediment Cum. (Outlet)" ))
