...
ws$Source <- ifelse(ws$variable == "DD13C.diss", "Outlet", 
                  ifelse(ws$variable == "DD13C.filt", "Outlet",
                         ifelse(ws$variable == "DD13C.South", "Plateau",
                                ifelse(ws$variable == "DD13C.Talweg", "Valley",
                                       "Plateau"))))
ws$Source <- as.factor(ws$Source)
levels(ws$Source)
ws$Source <- factor(ws$Source, levels = c("Plateau","Valley", "Outlet"))
...
wsPlot <- ggplot(data = ws, aes(x = Date.ti, y = value)) + 
  ...
  stat_smooth(data=subset(ws,
                          (Source != "Outlet" & Source != "Valley" & Event.x < 20 )), 
              method = "lm", formula = y ~  poly(x, 2), se = T, aes(colour = 'Plateau'), alpha = 0.1) +
  stat_smooth(data=subset(ws,
                          (Source != "Outlet" & Source == "Valley" & Event.x > 8 )), 
              method = "lm", formula = y ~ poly(x, 2), se = T, colour = 'green4' , alpha = 0.1) +
  stat_smooth(data=subset(ws,
                          (Source == "Outlet" & Event.x > 1 & Type == "Dissolved")), 
              method = "lm", formula = y ~ poly(x, 2), se = T, aes(colour = 'Outlet'), alpha = 0.1) +
...
wsPlot
ggsave(wsPlot, filename = "WateSoilvsTime.png", width = 8, height = 5, units = "in", scale = 1)
```
