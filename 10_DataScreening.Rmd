...
p <- ggplot(data = subset(soilGrApp, !is.na(ngC.Label)), aes(x=Conc.mug.g.dry.soil, y=DD13C.comp))+
  geom_point(aes(group = Transect, size = timeSinceApp, colour = Transect, shape = ngC.Label)) +
  theme_bw() +
  labs(size="Days after appl.", colour="Transect", shape = "Mass Carbon") +
...
raleigh <- ggplot(data = subset(soilGrApp, (Wnum < 12 & Wnum > 0 & !is.na(ngC.Label))), aes(x=xRaleigh, y=yRaleigh))+
  geom_point(aes(group = Transect, size = timeSinceApp, colour = Transect, shape = ngC.Label)) +
  theme_bw() +
  scale_size_continuous(range = c(1, 3)) +
  labs(size="Days post appl.", colour="Transect", shape = "Mass Carbon") +
  xlab("ln f") +
  ylab("ln R/Ro") +
  ylab(expression(paste("ln  ",  R / R['0'] ))) +
  stat_smooth(data= subset(soilGrApp , (Wnum < 12 & Wnum > 0 & !is.na(ngC.Label))), method = "lm", formula = y~x, se=T) +
  annotate("text", x = -0.5, y = 0.005, 
           label = as.character(expression(paste( "\u0190", " \u2030", " = ", -1.87, "\u00B1" , 0.43))), parse = T, size = 3.0) # +
...
waterRaleigh <- ggplot(data = subset(waterClean, (!is.na(yRaleigh))), aes(x=xRaleigh, y=yRaleigh))+
  ...
  ylab("ln R/Ro") +
  ylab(expression(paste("ln  ",  R / R['0'] ))) +
  stat_smooth(data= subset(waterClean, (!is.na(yRaleigh))), method = "lm", formula = y~x, se=T) 
...
```{r}
WaterSoils <- merge(waterClean, soils, by = "Date.ti", all = F)
str(WaterSoils)

limits_DdCsoil <- aes(ymin=comp.d13C-comp.d13C.SE-initialDelta, ymax=comp.d13C+comp.d13C.SE-initialDelta, colour = Transect)
...
#                       ifelse(wk$Date.ti == as.POSIXct('2016-04-14 13:52:00', tz = "EST") & wk$Transect == "T", 0.05, NA))))
  ...
  geom_point(aes(colour = Transect, shape = ngC.Label, shape = "Application"), size = 2) +
...
ggsave(x, filename = "xx.png", width = 8, height = 5, units = "in", scale = 1)

keepWS <- c("Date.ti",
            "DD13C.diss", "DD13C.filt",
            "DD13C.Talweg", "DD13C.South", "DD13C.North",
...
ws$Type <- ifelse(ws$variable == "DD13C.diss", "Dissolved", 
                  ifelse(ws$variable == "DD13C.filt", "Sediment",
                        "Top Soil"))

ws$Source <- ifelse(ws$variable == "DD13C.diss", "Outlet", 
                  ifelse(ws$variable == "DD13C.filt", "Outlet",
                         ifelse(ws$variable == "DD13C.South", "South",
                                ifelse(ws$variable == "DD13C.Talweg", "Talweg",
                                       "North"))))
ws$Source <- as.factor(ws$Source)
levels(ws$Source)
ws$Source <- factor(ws$Source, levels = c("Outlet" , "North", "Talweg", "South"))

ws$Event.x <- as.numeric(ws$Event.x)

# epsilon
epsilon_field <- cof

ggplot(data = ws, aes(x = Date.ti, y = value)) + 
  geom_point(aes(shape = Type, colour = Source)) + 
  theme_bw() + 
  # Applics
  ...
  stat_smooth(data=subset(ws,
                          (Source != "Outlet" & Event.x < 17 )), 
              method = "lm", formula = y ~  poly(x, 2), se = F, colour = "brown") +
  stat_smooth(data=subset(ws,
                          (Source == "Outlet" & Event.x > 1 & Type == "Dissolved")), 
              method = "lm", formula = y ~ poly(x, 2), se = F, colour = "dodgerblue") +
